...
// TODO: Compress into a vec4 if necessary
uniform float time;
...
void main()
{
    ...
    // TODO: Visualize pressure factor
    // float pressureFactor = (-forceDensityPressure.w - (gasConstant * restDensity - MAX_PRESSURE_DELTA)) / MAX_PRESSURE_DELTA;

    // Evaluate open simplex noise twice, using domain warping
    vec3 x = vec3(worldPos * causticFrequency / 32, time);
    vec4 noise = openSimplex2SDerivatives_Conventional(x);
    noise = openSimplex2SDerivatives_Conventional(x - noise.xyz / 16.0);
    float value = noise.w;

    // Fake wave caustic color contribution
    // NOTE: Less caustic effect when more density!
    vec3 causticContribution = causticColor.rgb * (0.5 + 0.5 * value) * causticColor.a * (1.0 - densityFactor);

    // Calculate the final color
    vec4 baseColor = vec4(fluidColor.rgb * (1.0 - densityFactor), fluidColor.a);
    vec4 waveColorContribution = vec4(waveColor.rgb * length(vel) / 20000 * waveColor.a, 0.0);
    finalColor = baseColor + waveColorContribution + vec4(causticContribution, 0.0);
}
